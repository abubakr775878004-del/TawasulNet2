    ...
    <div className="app">
      ...
      <div className="main">
        ...
        {profile.personal_card && (
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 12, color: '#E3D6FF', fontWeight: '700', marginBottom: 4 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
          ...
          <div style={{
            ...
            borderRadius: 20, padding: 20, color: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
            boxShadow: '0 10px 25px rgba(0, 0, 0, 0.1)', transition: 'background 0.3s ease'
          }}>
            <div>
        ...
        <div className="panel">
          ...
          {Object.keys(byPackage).length === 0 && (
            <div style={{ color: 'var(--ink-soft)', fontSize: 13 }}>لا توجد كروت لديك حاليًا</div>
          )}
        ...
        <div className="panel" style={{ marginTop: 20 }}>
          ...
          <form onSubmit={sendNoteToAdmin}>
            <textarea
              ...
              style={{ width: '100%', padding: 12, borderRadius: 10, border: '1.5px solid var(--line)', marginBottom: 10, fontSize: 13.5, resize: 'vertical' }}
            />
            {noteMessage && (
      ...
      {pendingPackage && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,10,40,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 22, maxWidth: 340, width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(120deg, #5B21B6, #7C3AED, #DB2777)', padding: '26px 20px 22px', color: '#fff' }}>
              <div style={{ fontSize: 12, color: '#E3D6FF', fontWeight: '700', marginBottom: 6 }}>إظهار كرت من باقة</div>
              <div style={{ fontSize: 26, fontWeight: '900' }}>{pendingPackage.name}</div>
            </div>
            <div style={{ padding: '20px 24px 24px' }}>
              <div style={{ fontSize: 12.5, color: 'var(--ink-soft)', marginBottom: 15, textAlign: 'right' }}>
                ...
                <input
                  ...
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid var(--line)', fontSize: '13px' }}
                />
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  disabled={revealBusy}
                  style={{ flex: 1, padding: '13px 0', borderRadius: 12, border: '1.5px solid var(--line)', background: '#fff', fontWeight: '800', cursor: 'pointer' }}
                >
                  إلغاء
                </button>
                <button
                  ...
                  disabled={revealBusy}
                  style={{ flex: 1, padding: '13px 0', borderRadius: 12, border: 'none', background: 'linear-gradient(120deg, #7C3AED, #DB2777)', color: '#fff', fontWeight: '800', cursor: 'pointer' }}
                >
      ...
      {revealedCard && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,10,40,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 24, maxWidth: 380, width: '100%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
            <div style={{ background: 'linear-gradient(120deg, #5B21B6, #7C3AED, #DB2777)', padding: '18px 20px', color: '#fff', position: 'relative' }}>
              <button
                onClick={closeModal}
                style={{ position: 'absolute', top: 12, left: 12, width: 30, height: 30, borderRadius: 10, border: 'none', background: 'rgba(255,255,255,0.25)', color: '#fff', fontWeight: '900', cursor: 'pointer' }}
              >
                ✕
              </button>
              <div style={{ fontSize: 12.5, color: '#E3D6FF', fontWeight: '700' }}>{revealedCard.packageName}</div>
              <div style={{ fontSize: 12, fontWeight: '900', marginTop: 2 }}>✓ تم البيع بنجاح</div>
            </div>
            <div style={{ padding: 26 }}>
              <div className="mono" style={{ fontSize: 28, fontWeight: '900', margin: '4px 0 18px', direction: 'ltr', color: '#3A1D66' }}>
                {revealedCard.code}
              </div>
              <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
                <button
                  onClick={copyCode}
                  style={{ flex: 1, padding: '11px 0', borderRadius: 12, border: '1.5px solid #DDD3F5', background: '#F3F0FB', color: '#5B21B6', fontWeight: '800', cursor: 'pointer' }}
                >
                  {copied ? '✓ تم النسخ' : '📋 نسخ الكود'}
                </button>
                <button
                  onClick={shareWhatsapp}
                  style={{ flex: 1, padding: '11px 0', borderRadius: 12, border: 'none', background: '#25D366', color: '#fff', fontWeight: '800', cursor: 'pointer' }}
                >
                  واتساب
                </button>
              </div>
              <button
                onClick={closeModal}
                style={{ width: '100%', padding: '13px 0', borderRadius: 14, border: 'none', background: '#F3F0FB', color: '#5B21B6', fontWeight: '800', cursor: 'pointer' }}
              >
                إغلاق
              </button>
            </div>
