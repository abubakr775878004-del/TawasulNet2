...
export default function DistributorPage() {
  ...
  // حالة لتخزين المبلغ الصافي المستحق للمدير (90% من إجمالي المبيعات - المدفوعات)
  ...
  async function load() {
    ...
    try {
      ...
      // 4. حساب الدين المالي الدقيق للمدير: (إجمالي أسعار الكروت المباعة × 90%) - المدفوعات المسجلة
  ...
  async function confirmReveal() {
    ...
    try {
      ...
      // تسجيل العملية في سجل المبيعات مع السعر الكامل (أو سعر الباقة لتتمكن النسبة من الحساب في الـ backend أو الـ frontend بدقة)
    ...
    <div className="app">
      ...
      <div className="main">
        <div className="topbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: isOnline ? '#ECFDF5' : '#FEF2F2', color: isOnline ? '#059669' : '#DC2626', padding: '6px 12px', borderRadius: 20, fontSize: 11.5, fontWeight: '800' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: isOnline ? '#10B981' : '#EF4444' }}></span>
            {isOnline ? 'نشط' : 'خامل'}
          </div>
        </div>
        ...
        {/* بطاقات الأرصدة والمستحقات مع حساب 90% للمدير */}
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 12, color: '#f1f5f9', fontWeight: '700', marginBottom: 6 }}>
                المبلغ الصافي المستحق للمدير (90%)
              </div>
        ...
        <div className="panel">
          <div className="panel-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={load} disabled={isRefreshing} style={{ background: '#F3F0FB', border: '1px solid #DDD3F5', color: '#5B21B6', padding: '6px 12px', borderRadius: '10px', fontSize: '12px', fontWeight: '800', cursor: 'pointer' }}>
              {isRefreshing ? 'جاري التحديث...' : '🔄 تحديث القائمة'}
            </button>
          </div>

          {revealError && <div style={{ color: '#DC2626', background: '#FEF2F2', padding: '10px', borderRadius: '8px', marginBottom: '10px', fontSize: '13px' }}>{revealError}</div>}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,10,40,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 22, maxWidth: 340, width: '100%', textAlign: 'center', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.35)' }}>
            ...
            <div style={{ padding: '20px 24px 24px', textAlign: 'right' }}>
              <label style={{ display: 'block', marginBottom: 6, fontWeight: '700', color: '#374151', fontSize: '12.5px' }}>
                اسم الزبون (اختياري للسحب الأسبوعي):
              </label>
              <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="مثال: أحمد محمد" style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid var(--line)', fontSize: '13px', outline: 'none', marginBottom: 15 }} />
      ...
      {revealedCard && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,10,40,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 24, maxWidth: 380, width: '100%', textAlign: 'center', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.35)' }}>
            <div style={{ background: 'linear-gradient(120deg, #5B21B6, #7C3AED, #DB2777)', padding: '18px 20px', color: '#fff' }}>
            ...
            <div style={{ padding: 26 }}>
              <div className="mono" style={{ fontSize: 28, fontWeight: '900', margin: '4px 0 18px', letterSpacing: 1, direction: 'ltr', color: '#3A1D66' }}>
                {revealedCard.code}
              </div>
              ...
              <button onClick={closeModal} style={{ width: '100%', padding: '13px 0', borderRadius: 14, border: 'none', background: '#F3F0FB', color: '#5B21B6', fontWeight: '800', cursor: 'pointer' }}>إلغاء وإغلاق</button>
            </div>
          </div>
        </div>
      )}
