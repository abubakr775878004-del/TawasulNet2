...
function getPersonalRecordCutoff() {
...
export default function DistributorSalesPage() {
  ...
  const [personalSoldCards, setPersonalSoldCards] = useState([]);
  ...
  const [deletingPersonalRecordId, setDeletingPersonalRecordId] =
    useState(null);
  ...
  async function loadData(showRefresh = false) {
    ...
    try {
       ...
       * العمولة
       *
       * يعرض آخر 30 يومًا فقط.
       */
      const personalCutoff =
        getPersonalRecordCutoff();

      const {
        data: personalSoldData,
        error: personalSoldError
      } = await supabase
        .from('distributor_sold_cards')
        .select(
          'id, card_code, customer_name, package_name, price, sold_at, created_at'
        )
        .eq('distributor_id', profile.id)
        .gte('sold_at', personalCutoff)
        .order('sold_at', {
          ascending: false
        });

      if (personalSoldError) {
        console.error(
          'Error loading personal sold cards:',
          personalSoldError
        );
      }

      setPersonalSoldCards(
        personalSoldData || []
      );
       ...
       * =====================================================
       *
       * with_distributor لا يدخل في المبيعات التاريخية.
       */
      const {
        data: inventoryData,
   ...
   * - أي سجل مالي
   */
  async function deletePersonalSoldCard(recordId) {
    if (!recordId || !profile) return;

    const confirmed = window.confirm(
      'هل تريد حذف هذا الكرت من سجلك الشخصي فقط؟\n\nلن يتم حذف الكرت الأصلي أو عملية البيع أو أي بيانات مالية.'
    );

    if (!confirmed) return;

    setDeletingPersonalRecordId(recordId);
    ...
    try {
      ...
      if (error) {
        console.error(
          'Error deleting personal sold card:',
          error
        );

        window.alert(
          'تعذر حذف السجل من سجلك الشخصي.'
        );

        return;
      }

      setPersonalSoldCards((current) =>
        current.filter(
      ...
      console.error(
        'Personal sold card deletion error:',
        error
      );
      ...
      setDeletingPersonalRecordId(null);
    }
  }
  ...
  /*
   * عدد العمليات التي لا تحتوي
   * على عمولة تاريخية محفوظة.
   */
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        >
          ...
          >
            هذا السجل خاص بك فقط. حذف سجل من هنا لا يحذف
            الكرت الأصلي ولا عملية البيع ولا يؤثر على الدين
            أو العمولة أو التقارير المالية.
          </div>

          {personalSoldCards.length === 0 ? (
            ...
            >
              ...
              >
                لا توجد كروت مباعة مسجلة خلال آخر 30 يومًا
              </div>
              ...
              >
                سيظهر هنا الكرت بعد إتمام عملية البيع.
              </div>
            </div>
