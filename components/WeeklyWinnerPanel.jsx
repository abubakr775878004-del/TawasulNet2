...
export default function DistributorWeeklyWinner({
  ...
  async function fetchWinnersAndCheckTime() {
    try {
       ...
       *
       * هذا يمنع اختلاف حساب الأسبوع بين الواجهة
       * وقاعدة البيانات.
       */
      const {
        data: weekKey,
      ...
      if (weekError || !weekKey) {
        console.error(
          'Error getting weekly draw week:',
          weekError
        );
      ...
      /*
       * الاعتماد على expires_at الرسمي الموجود
       * في weekly_winners بدل حساب المدة من created_at.
       */
      ...
      console.error(
        'Unexpected error fetching weekly winners:',
        err
      );
  ...
  async function handleSendNotificationOnly() {
    if (winners.length === 0) {
      setMessage(
        '⚠️ لا توجد نتائج فائزين لإرسالها حالياً'
      );
      return;
    }
...
🥇 المركز الأول: ${
        winners[0]?.customer_name || '—'
      } (الموزع: ${
        winners[0]?.distributor_name || '—'
      })
🥈 المركز الثاني: ${
        winners[1]?.customer_name || '—'
      } (الموزع: ${
        winners[1]?.distributor_name || '—'
      })
🥉 المركز الثالث: ${
        winners[2]?.customer_name || '—'
      } (الموزع: ${
        winners[2]?.distributor_name || '—'
      })
  ...
  return (
    <div
      style={{
        ...
        color: '#fff',
        boxShadow:
          '0 10px 25px rgba(49, 46, 129, 0.2)',
        marginBottom: '20px',
      }}
    >
      <div
      ...
      {isTimeToShow ? (
        ...
        >
          🔒 سيظهر أسماء الفائزين الثلاثة فور
          إعلان القرعة ولمدة{' '}
          <strong>30 ساعة</strong>. استمر في بيع
          الكروت لزيادة فرصة زبائنك!
        </div>
      )}
    </div>
  );
...
ومهم جدًا: هذه النسخة وحدها لا تكفي؛ يجب أن يكون استدعاؤها في "app/admin/page.js" هكذا:

<WeeklyWinnerPanel isAdmin={profile?.role === 'admin'} />

ولا نحتاج تعديل "useProfile" أو قاعدة البيانات أو دالة السحب.
